...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch("add_global_pto.php", {
        method: "POST",
        body: formData
      });
      ...
      if (data.success) {
        // ✅ Update the accordion dynamically
        addOrUpdateGroup(data.entry);
  ...
  function addOrUpdateGroup(entry) {
    // Find group card by note
    let existingCard = document.querySelector(`[data-note='${CSS.escape(entry.timeoff_note)}']`);

    if (existingCard) {
      // ✅ Update existing group
      let hoursEl = existingCard.querySelector(".group-hours");
      let weeksEl = existingCard.querySelector(".group-weeks");

      let currentHours = parseInt(hoursEl.textContent) || 0;
      hoursEl.textContent = (currentHours + parseInt(entry.assigned_hours)) + " hrs";

      weeksEl.textContent += ", " + entry.week_start;

      // Optionally, append a new row into accordion body
      let body = existingCard.querySelector(".card-body");
      ...
      const cardHTML = `
        <div class="card shadow-sm global-pto-card" data-note="${entry.timeoff_note}" style="border-radius: 6px; border: 1px solid #e0e0e0;">
          <div class="card-header d-flex justify-content-between align-items-center" style="cursor:pointer; height: 85px;" data-bs-toggle="collapse" data-bs-target="#collapse${newIndex}">
            <div>
              <p class="mb-1 fs-6 fw-semibold text-capitalize">${entry.timeoff_note || "No Note"}</p>
