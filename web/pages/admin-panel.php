...
emailForm.addEventListener('submit', async (e) => {
  ...
  // Flatten nested notification_types checkboxes
  const notificationTypes = {};
  formData.forEach((value, key) => {
    if (key.startsWith('notification_types[')) {
      const subKey = key.match(/notification_types\[(.+)\]/)[1];
      notificationTypes[subKey] = value === 'true' ? 'true' : 'false';
      delete data[key]; // remove original
    }
  });

  // Merge flattened notification_types into main settings object
  Object.keys(notificationTypes).forEach(k => {
    data[`notification_types[${k}]`] = notificationTypes[k];
  });
