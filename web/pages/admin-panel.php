    ...
    <div class="modal-content border-0 shadow-sm">
      ...
      <div class="modal-body">
        ...
        <div class="accordion d-flex flex-column gap-2" id="ptoAccordion">
          ...
          // Query all entries
          $sql = "
            SELECT timeoff_id, DATE_FORMAT(week_start, '%m/%d/%Y') AS week_start, assigned_hours, timeoff_note
            FROM time_off
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addForm.addEventListener("submit", async function(e) {
    ...
    try {
      ...
      if (data.success) {
        // ✅ Reload the accordion dynamically
        reloadPTOAccordion();
        addForm.reset();
      } else {
  ...
  document.querySelectorAll(".updatePTOForm").forEach(form => {
    form.addEventListener("submit", async function(e) {
      ...
      try {
        ...
        console.error(err);
        alert("Something went wrong while updating PTO.");
      }
    });
  });
  ...
  async function reloadPTOAccordion() {
    try {
      ...
      document.getElementById("ptoAccordion").innerHTML = html;

      // Reattach update handlers
      document.querySelectorAll(".updatePTOForm").forEach(form => {
        form.addEventListener("submit", async function(e) {
          e.preventDefault();
          const formData = new FormData(form);
          try {
            const res = await fetch("update_global_pto.php", { method: "POST", body: formData });
            const data = await res.json();
            if (data.success) reloadPTOAccordion();
