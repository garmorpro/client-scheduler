<?php
// Weekly status call day/group are on audit_engagement_timeline but aren't
// part of the fixed date/completed column whitelist in
// audit_timeline_fields.php (they're not a "due date" - a recurring weekday
// + an optional cross-engagement link group), so this gets its own small
// endpoint rather than being shoehorned into update_audit_timeline_field.php.
...
$stmt = $conn->prepare("UPDATE audit_engagement_timeline SET weekly_status_call_day = ?, weekly_status_call_group_name = ? WHERE engagement_id = ?");
$stmt->bind_param('isi', $dayValue, $groupValue, $engagementId);
$ok = $stmt->execute();
$stmt->close();
