...
$engagementQuery = "
    ...
    GROUP BY e.engagement_id, c.client_name, e.engagement_name, e.status, e.budgeted_hours, e.manager, e.notes, d.tsc, d.location, d.poc, d.scope, d.repeat_flag, d.soc_type, d.as_of_date, d.review_period_start, d.review_period_end, d.pci_assessment_type, d.pci_delivery_date
                            ...
                            <td class="num">
                                <div class="client-row-actions">
                                    ...
                                    <button class="client-icon-btn edit edit-engagement-btn"
                                        ...
                                        data-pci-assessment-type="<?php echo htmlspecialchars($row['pci_assessment_type'] ?? ''); ?>"
                                        data-pci-delivery-date="<?php echo htmlspecialchars($row['pci_delivery_date'] ?? ''); ?>"
                                        title="Edit">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="text" id="deleteEngagementConfirmInput" class="form-control" autocomplete="off" placeholder="DELETE">
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
<script>
    ...
    // Delete (permanent) flow - requires typing DELETE to confirm, since this
    // is irreversible and Archive already covers the "I don't need this
    // anymore but want to keep a record" case.
    ...
    deleteConfirmInput.addEventListener("input", function () {
        confirmDeleteEngagementBtn.disabled = deleteConfirmInput.value.trim().toLowerCase() !== "delete";
    });
