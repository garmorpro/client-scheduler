...
$headers = ['client_name', 'engagement_name', 'year', 'status', 'budgeted_hours', 'manager', 'audit_types', 'tsc', 'soc_type', 'as_of_date', 'review_period_start', 'review_period_end', 'pci_assessment_type', 'pci_delivery_date', 'location', 'poc', 'scope', 'repeat', 'notes'];
$sheet->fromArray($headers, null, 'A1');
styleHeaderRow($sheet, count($headers));
...
$sheet->getStyle('A2:S2')->getFont()->setItalic(true)->getColor()->setRGB('9AA39D');
...
// Not every PCI engagement produces a ROC (Report on Compliance) - smaller
// merchants/service providers instead get an AOC or file a SAQ variant.
addDropdown($sheet, 'M', 2, 200, ['ROC', 'AOC', 'SAQ A', 'SAQ A-EP', 'SAQ B', 'SAQ B-IP', 'SAQ C', 'SAQ C-VT', 'SAQ D (Merchant)', 'SAQ D (Service Provider)', 'P2PE']);
addDropdown($sheet, 'R', 2, 200, ['Yes', 'No']);
foreach (['J', 'K', 'L', 'N'] as $col) $sheet->getStyle("{$col}2:{$col}200")->getNumberFormat()->setFormatCode('yyyy-mm-dd');
foreach (['A' => 28, 'B' => 22, 'C' => 8, 'D' => 14, 'E' => 15, 'F' => 20, 'G' => 22, 'H' => 26, 'I' => 10, 'J' => 13, 'K' => 16, 'L' => 16, 'M' => 22, 'N' => 16, 'O' => 16, 'P' => 16, 'Q' => 24, 'R' => 9, 'S' => 24] as $col => $w) $sheet->getColumnDimension($col)->setWidth($w);
