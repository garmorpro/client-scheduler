...
$permissionKeys = ['manage_employees', 'manage_clients_engagements', 'approve_time_off', 'access_system_settings'];
...
try {
    $stmt = $conn->prepare("
        ...
        SET manage_employees = ?, manage_clients_engagements = ?, approve_time_off = ?, access_system_settings = ?
        WHERE role = ?
    ");
    foreach ($permissions as $entry) {
        ...
        $stmt->bind_param('iiiis', $values[0], $values[1], $values[2], $values[3], $role);
        $stmt->execute();
    }
    $stmt->close();
