...
> **Mirror copy.** The canonical version of this doc lives in the Engagement Tracker repo at `docs/client-scheduler-migration-plan.md` (also published as a Claude Artifact: https://claude.ai/code/artifact/13ffdb63-75e1-4910-ad38-1f6706c121a2). This copy was placed here on 2026-08-13 so it's readable directly from the Client Scheduler repo/laptop without needing the other repo checked out too. Update both when this changes, or treat Engagement Tracker's copy as the source of truth if they ever drift.

> ⏸️ **PAUSED as of 2026-09-08.** Garrett confirmed he's still working `engagements.morganserver.com` (Engagement Tracker) as his main day-to-day place for engagements — this migration is on hold, not moving forward for now. Nothing below was reversed or undone; Phases 0-2 and most of Phase 3 are still committed and live on this repo exactly as they were left. **Note:** the `audit-tracking-migration` branch mentioned below has since been merged into `main` (outside of any session tracked here) — `main` now has all of this migration work *plus* newer, unrelated feature work (Onboard Employee wizard, Intern manager assignment) that happened after the migration paused. Treat `main` as current, not the now-stale `audit-tracking-migration` branch. If/when this resumes, start by re-reading the "Pick up here" section below — it was accurate as of 2026-08-13 and nothing about the migration itself has changed since.
...
7. ~~**A now-orphaned Bootstrap "Add Engagement" modal** (`includes/modals/add_engagement_modal.php` + `assets/js/add_engagement_modal.js`) is still sitting in the codebase, confirmed dead/unreachable — flagged as a background cleanup task, not yet done.~~ **Done.** Confirmed via repo-wide grep that nothing referenced `data-bs-target="#addEngagementModal"` — the live `.add-engagement-btn` flow is `assets/js/swal-modals/add-engagement-modal.js` instead. Deleted both files and their two include/script lines in `client-management.php`.
8. **Fixed (2026-08-13): new/restored engagements were silently missing their Timeline & Key Dates card.** `add_engagement.php` only ever inserted an `audit_engagement_details` row when TSC happened to be set, and never inserted an `audit_engagement_timeline` row at all; `unarchive_engagement.php` inserted neither when restoring an archived engagement into a fresh `engagements` row. Since `renderTimelineSection()` returns nothing at all when the backend finds no timeline row (not just an empty card — the whole card is absent), this meant any engagement added or restored since Phase 2 landed was quietly missing the card, while migrated engagements (which all got a row from the one-time backfill) had it. Both write paths now always insert a bare row (all dates NULL) alongside `engagements`. **Still needs running on the server:** `storage/migrations/2026-08-13_backfill_missing_audit_rows.sql` — backfills the missing rows for every engagement created/restored before this fix. Safe to re-run (`INSERT IGNORE`).
...
4. **"My engagements" ✅ done.** Mostly fell out of step 2 for free, since `my-schedule.php` already uses the shared modal. The remaining piece — timeline items checkable by anyone holding `complete_audit_timeline_items`, due dates editable only by `manage_audit_timeline` — is built: `engagement-details.php` returns two per-engagement-scoped flags (`can_manage_timeline`, `can_complete_timeline` — admin passes for any engagement, everyone else needs the role permission *and* an `entries` row on that specific engagement), and the modal renders date inputs / a checkable dot accordingly. Two new endpoints (`update_audit_timeline_field.php`, `update_audit_milestone.php`) handle the writes, both keyed off a shared column whitelist (`includes/audit_timeline_fields.php`). DOL was already read-only under `view_dol` from step 2.
...
6. Restyle to AARC-360's palette (`#003f47` / `#a3cc38`) — cosmetic, do last. **View Engagement done and taken further than planned (2026-08-13):** converted from a centered Bootstrap modal into a right-side slide-in panel (Bootstrap Offcanvas) matching Engagement Tracker's reference panel layout almost exactly, not just its color/section styling. Sectioned cards (colored dot + label — Overview/Capacity/Details/Notes/Assigned Employees/Timeline/Milestones/Team (DOL)/Independence) as before, plus: an engagement-code header (`ENG-2026-013` style) and large client name; an inline status dropdown for anyone who can manage the engagement (Client Scheduler's own `confirmed`/`pending`/`not_confirmed` values — deliberately *not* Engagement Tracker's "In Progress" vocabulary, which doesn't exist in this schema); Edit/Archive/Delete buttons in the header (Delete goes through a type-DELETE-to-confirm nested modal, same pattern as the other permanent-delete flows); red overdue dots on timeline/milestone rows past their due date and not yet complete; an editable Weekly Status Call day; a Manage Team link into `dol-generator.php`; and Upload/Download Planning Doc (new `storage/planning_docs/` safe-root, mirroring the backup-file pattern). New endpoints: `update_engagement_status.php`, `update_audit_weekly_call.php`, `upload_planning_doc.php`, `download_planning_doc.php`. All per-engagement writes reuse `audit_can_act_on_engagement()` (admin, or staffed via `entries`); Edit/Archive/Delete use the existing non-scoped `manage_clients_engagements` check, consistent with how those already worked elsewhere.
   - **Gaps left open, not yet surfaced anywhere in the UI:** no way to edit Location/POC/Scope/Review Period/As-of Date/Repeat flag from any screen — `edit_engagement.php`/its modal only ever touched budgeted hours/status/manager/notes/audit types/TSC, and the new panel renders those `audit_engagement_details` fields read-only.
   - ~~**Deliberately not built:** a separate "Import Timeline" / "Edit Timeline" mode-toggle~~ **"Edit Timeline" toggle added (2026-08-13)**, matching the reference exactly: dates are read-only by default (label-over-date, colored status dot — green done / red overdue / gray upcoming — click a row to toggle complete/incomplete), and "Edit Timeline" swaps the same card into the old inline `<input type="date">` boxes for whoever has `manage_audit_timeline` on that engagement. Toggling is a local re-render (no network round trip) that survives a save-triggered refresh via a module-scoped flag, keyed off engagement_id. "Import Timeline" (bulk-paste) and "+ Link engagement" (linking related engagements together) from the reference are still not built — no backing capability for either yet.
   - ~~**Known rough edge:** "Manage Team" links to `dol-generator.php?engagement_id=X`, but that page doesn't currently read an `engagement_id` query param.~~ **Fixed (2026-08-13).** `dol-generator.php` now reads it and pre-selects the matching `<option>` (only if it's actually in that user's own admin-or-staffed engagement list), and `dol_generator.js` fires the picker's existing change handler on load so the page auto-loads straight into that engagement's setup.

   DOL Generator, Training page, and Audit Calendar are still on their original "functional first pass" styling.
