...
6. Restyle to AARC-360's palette (`#003f47` / `#a3cc38`) — cosmetic, do last. **View Engagement modal done**, ahead of the rest: rebuilt as sectioned cards (colored dot + label per section - Overview/Capacity/Details/Notes/Assigned Employees/Timeline/Milestones/Team (DOL)/Independence) using the Engagement Tracker reference screenshot as the layout target, entirely in the app's existing palette. Also surfaced `audit_engagement_details` (location, POC, review period, report/SOC type, TSC, scope) and audit type chips, neither of which were in the modal before. DOL Generator, Training page, and Audit Calendar are still on their original "functional first pass" styling.
